...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: Yonetici_Panel.php?tab=attendance_records&msg=Kayıt güncellendi");
    exit();
}

// Mevcut yoklama kaydını çek
$stmt = $db->prepare("SELECT * FROM attendance_records WHERE id = ?");
$stmt->execute([$id]);
$row = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$row) {
    die("Yoklama kaydı bulunamadı!");
}

// Yoklama listesi ve öğrenci listesi
...
<head>
  ...
  <title>Yoklama Kaydı Düzenle - Yönetici Paneli</title>
  <link
    rel="stylesheet"
...
<body>
  <div class="edit-container">
    <div class="edit-header">
      ...
      <h1>Yoklama Kayıdı Düzenle</h1>
      <p>Öğrenci Öğretmen Yönetim Sistemi</p>
    </div>
    <div class="edit-form">
      <form method="POST" action="">
        <div class="form-group">
          <label for="attendance_id">Yoklama Tarihi</label>
          <div class="input-icon">
            <i class="fas fa-calendar-day"></i>
            <select id="attendance_id" name="attendance_id" class="form-control" required>
              <?php foreach ($attendances as $a): ?>
                <option value="<?= $a['id'] ?>" <?= $row['attendance_id'] == $a['id'] ? 'selected' : '' ?>>
                  <?= htmlspecialchars($a['attendance_date']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
      ...
      <div class="back-button">
        <a href="Yonetici_Panel.php?tab=attendance_records">
